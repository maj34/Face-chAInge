...
class RetinaFace(nn.Module):
    ...
    def forward(self,inputs):
        ...
        gender_classifications = torch.cat([self.GenderHead[i](feature) for i, feature in enumerate(features)], dim=1)
        age_classifications = torch.cat([self.AgeHead[i](feature) for i, feature in enumerate(features)], dim=1)

        if self.phase == 'train':
            output = (bbox_regressions, classifications, ldm_regressions, gender_classifications, age_classifications)
        else:
            output = (bbox_regressions, F.softmax(classifications, dim=-1), ldm_regressions, F.softmax(gender_classifications, dim = -1), F.softmax(age_classifications, dim = -1))
        return output